...
function LocalTripHistory() {
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useQuery(["favorites", status], () => fetchFavorites(status));

  const formatPrice = (tour) => {
    const totalCost = tour.costDetails.$values.reduce(
    ...
    const totalActivityAmount = tour.itinerary.$values.reduce((sum, day) => {
      return (
        sum +
        day.activities.$values.reduce(
          (activitySum, activity) => activitySum + activity.activityAmount,
          0
        )
      );
    }, 0);
  ...
  const handleEditClick = async (tourId) => {
    const response = await axios.get(
      `https://travelmateapp.azurewebsites.net/api/Tour/${tourId}`
    );
  ...
  const displayedFavorites =
    loading || !dataProfile || !dataProfile.$values ? [] : dataProfile.$values;
  ...
  useEffect(() => {
    if (selectedFavorite) {
      ...
      setStartDate(
        new Date(selectedFavorite.startDate).toISOString().slice(0, 16)
      );
  ...
  const handleUpdateTour = () => {
    const totalCost = costDetails.reduce(
      ...
      return (
        ...
        day.activities.$values.reduce(
          (activitySum, activity) =>
            activitySum + parseFloat(activity.activityAmount),
          0
        )
      );
    }, 0);
    ...
    axios
      .put(
        `https://travelmateapp.azurewebsites.net/api/Tour/${selectedFavorite.tourId}`,
        formData
      )
      .then((response) => {
  ...
  useEffect(() => {
    if (startDate && endDate) {
      ...
      if (start <= end) {
        ...
        setDuration(days); // Cập nhật số ngày
        adjustItinerary(days); // Điều chỉnh lịch trình
      }
    }
  }, [startDate, endDate]);
  const adjustItinerary = (days) => {
    const updatedItinerary = [...itinerary];

    // Nếu số ngày tăng, thêm ngày mới
    while (updatedItinerary.length < days) {
      updatedItinerary.push({
        day: updatedItinerary.length + 1,
        date: new Date(
          new Date(startDate).getTime() +
            updatedItinerary.length * (1000 * 60 * 60 * 24)
        ).toISOString(),
        activities: { $values: [] },
      });
    }

    // Nếu số ngày giảm, cắt bớt ngày
    while (updatedItinerary.length > days) {
  ...
  const handleActivityImageChange = (dayIndex, activityIndex, e) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const updatedItinerary = [...itinerary];
      updatedItinerary[dayIndex].activities.$values[
        activityIndex
      ].activityImage = URL.createObjectURL(file);
      setItinerary(updatedItinerary);
    }
  };
  ...
  const handleActivityImageClick = (dayIndex, activityIndex) => {
    document
      .getElementById(`activity-image-input-${dayIndex}-${activityIndex}`)
      .click();
  };
  ...
  return (
    <Container className="py-3 px-0 border-0 rounded-5">
      ...
      {loading ? (
        <div>
          {[...Array(4)].map((_, index) => (
            <Row
              key={index}
              className="border-bottom rounded p-3 mb-3 bg-white"
            >
                    ...
                    <Col md={4}>
                      ...
                      <p className="d-flex align-items-center">
                        <ion-icon
                          ...
                          style={{ marginRight: "8px" }}
                        ></ion-icon>{" "}
                        {favorite.numberOfDays} ngày {favorite.numberOfNights}{" "}
                        đêm
                      </p>
                    </Col>
                    ...
                    <Col md={1} className="">
                      <Dropdown>
                        ...
                        <Dropdown.Menu>
                          ...
                          <Dropdown.Item
                            onClick={() => handleEditClick(favorite.tourId)}
                          >
                            Chỉnh sửa
                          </Dropdown.Item>
                          <Dropdown.Item
                            onClick={() => handleDeleteTour(favorite.tourId)}
                          >
                            Xóa
                          </Dropdown.Item>
                        </Dropdown.Menu>
      ...
      <Modal
        ...
        dialogClassName="modal_extraDetailForm"
      >
        <Modal.Body>
          <div className="w-100">
            ...
            <Row>
              ...
              <Col lg={8} className="form_create_tour border-1 p-3 rounded-4">
                ...
                <Form.Group className="d-flex align-items-center mb-3">
                  ...
                  <Form.Control
                    type="datetime-local"
                    placeholder="Chọn thời gian diễn ra"
                    value={startDate}
              ...
              <Col lg={4}>
                <div
                  className="upload_image_create_trip d-flex align-items-center justify-content-center"
                  onClick={() => {
                    document
                      .getElementById("upload_trip_img_detail_update")
                      .click();
                  }}
                >
                  {tourImage ? (
            ...
            >
                      ...
                      >
                        ...
                        <div className="timeline_day_container">
                          {day.activities.$values.map((activity, idx) => (
                            <div
                              className="w-100 border-1 rounded-3 mb-3 py-3 p-4"
                              key={idx}
                            >
                              <div className="w-100 d-flex justify-content-end">
                                <Button
                                  variant="outline-danger"
                                  onClick={() => removeActivity(index, idx)}
                                  className="mb-2"
                                >
                                  Xóa
                                </Button>
                              ...
                              <div className="d-flex gap-3">
                                <img
                                  ...
                                  height={250}
                                  onClick={() =>
                                    handleActivityImageClick(index, idx)
                                  }
                                ...
                                <div className="w-100">
                                  <div className="d-flex gap-3 mb-2">
                                    ...
                                    <Form.Control
                                      ...
                                      className="d-none"
                                      onChange={(e) =>
                                        handleActivityImageChange(index, idx, e)
                                      }
                                    />
                                    <Form.Control
                                      type="number"
                ...
                <Row>
                  ...
                  <Col lg={5}>
                    <Form.Group className="d-flex flex-column">
                      ...
                      <TextareaAutosize
                        minRows={5}
                        className="rounded-3 p-2 fw-normal"
                  ...
                  {costDetails.map((cost, index) => (
                    <Col lg={12} key={index}>
                      <div className="money_item p-2 w-100">
                        ...
                        <p className="w-50" style={{ fontSize: "12px" }}>
                          {cost.notes}
                        </p>
                      </div>
                    </Col>
              ...
              <Tab eventKey="contact" title="QUY ĐỊNH">
                ...
                <div className="d-flex gap-2 mt-3">
                  <input type="checkbox" />
                  <div className="m-0">
                    Bằng cách tạo tour này, bạn xác nhận rằng đã đọc, hiểu và
                    đồng ý với các{" "}
                    <Link
      ...
      <input
        type="file"
        id="upload_trip_img_detail_update"
        className="d-none"
      ...
      <Modal show={showUpdateMoney} centered onHide={handleCloseUpdateMoney}>
        ...
        <Modal.Footer>
          <Button
            variant="secondary"
            className="rounded-5"
            onClick={handleCloseUpdateMoney}
          >
            Đóng
          </Button>
          <Button
